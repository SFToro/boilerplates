...
if grep -E -q "^LABEL=${FS_LABEL}\s" /etc/fstab; then  # ...this script already ran.
    exit 0
fi
...
readonly DISK_DEVICE=$(sudo parted /dev/sda print devices | grep -E '^/dev/sd[b-z]\s' | cut -f1 -d' ' | sort | tail -1)
...
if ! sudo file -s /dev/sdc | grep -E -q "^${DISK_DEVICE}: data$"; then
...
echo -e "LABEL=${FS_LABEL}\t${FS_MOUNT_POINT}\t${FS_TYPE}\tdefaults\t0 0" | sudo tee -a /etc/fstab >/dev/null 2>&1
